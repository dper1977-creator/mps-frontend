document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderDonasiTable(data) {
        ...
        data.forEach(donasi => {
            ...
            row.innerHTML = `
                ...
                <td>${donasi.alamat || '-'}</td>
                <td>${donasi.no_hp || '-'}</td>
                <td>${formatCurrency(donasi.nominal)}</td>
                <td>${donasi.nama_jenis}</td>
                <td>
                    <button class="btn edit" data-id="${donasi.no}">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn danger" data-id="${donasi.no}">
    ...
    function renderStatsTable(stats) {
        ...
        stats.forEach(stat => {
            const row = document.createElement('tr');
            row.innerHTML = `
                <td>${stat.nama_jenis}</td>
                <td>${stat.jumlah_donasi}</td>
                <td>${formatCurrency(stat.total_nominal)}</td>
                <td>${formatCurrency(stat.rata_rata)}</td>
            `;
